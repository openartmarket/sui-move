...
describe("splitContractStock", () => {
	...
	it("should split a split stock", async () => {
		...
		const { splitContractStockId: splitAgainContractStockId } =
			await splitContractStock(wallet, {
		...
		expect(await getWalletQuantity(wallet, splitContractStockId)).toEqual(2);
		expect(await getWalletQuantity(wallet, splitAgainContractStockId)).toEqual(
			3,
		);
	}, 30_000);
});
