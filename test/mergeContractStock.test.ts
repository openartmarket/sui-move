...
describe("mergeContractStock", () => {
	...
	it("should merge contract stocks", async () => {
		const { contractStockId: toContractStockId } = await mintContractStock(
			adminWallet,
			{
		...
		const { contractStockId: fromContractStockId } = await mintContractStock(
			adminWallet,
			{
				adminCapId: ADMIN_CAP_ID,
				contractId,
		...
		await expect(
			getWalletQuantity(wallet, fromContractStockId),
