...
if [ "${SUI_NETWORK:-localnet}" = "localnet" ]; then
    sui client test-publish --build-env localnet --gas-budget 200000000 --json ./move/coownable > tmp/publish.res.json
else
    sui client publish --gas-budget 200000000 --json ./move/coownable > tmp/publish.res.json
fi
...
ADMIN_CAP_ID=$(cat tmp/changes.res.json | jq -r 'select (.objectType | contains("::asset::AdminCap")).objectId')
...
direnv exec . pnpm exec tsx ./scripts/publish-displays.ts
