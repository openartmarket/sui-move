...
async function startMotion(wallet, params) {
  ...
  const response = await wallet.execute(async (txb, packageId) => {
    txb.moveCall({
      ...
      arguments: [txb.object(adminCapId), txb.pure.string(contractId), txb.pure.string(motion)]
    });
  });
  const { digest } = response;
  const createdObjects = getCreatedObjects(response);
...
var import_sui2 = require("@shinami/clients/sui");

// src/wallets.ts
var import_bcs = require("@mysten/bcs");
var import_transactions = require("@mysten/sui/transactions");
var import_sui = require("@shinami/clients/sui");
var SuiWallet = class {
  constructor(params) {
    this.params = params;
  }
  ...
  async execute(build) {
    const txn = new import_transactions.Transaction();
...
var ShinamiWallet = class {
  ...
  async execute(build) {
    ...
    const { signature } = await signer.signTransaction((0, import_bcs.fromBase64)(sponsoredResponse.txBytes));
...
async function newWallet(params) {
  switch (params.type) {
    ...
    case "shinami": {
      ...
      const suiClient = (0, import_sui2.createSuiClient)(shinamiAccessKey);
      return new SuiWallet({
        packageId,
        suiClient,
        keypair
    ...
    case "shinami-sponsored": {
      ...
      const suiClient = (0, import_sui2.createSuiClient)(shinamiAccessKey);
      const gasClient = new import_sui2.GasStationClient(shinamiAccessKey);
      const keyClient = new import_sui2.KeyClient(shinamiAccessKey);
      const walletClient = new import_sui2.WalletClient(shinamiAccessKey);
